...
The designation $\gamma$ is equivalent to   $\{\lambda l L S\}$.
...
%(\ref{basis_function_1}) and (\ref{basis_function_2})
...
\caption{\label{m3y_potpar}  Parameters of the M3Y-Paris \cite{anantaraman1983effective}  NN-potential with the Direct and Exchange terms and depending on the isospin T terms.}
...
\begin{equation}
V^{DF}({\bf r})=V^{DF}_{\alpha+\alpha}({\bf r})+V^{DF}_{2N+\alpha}({\bf r})
\label{df_in_terms}
\end{equation}
