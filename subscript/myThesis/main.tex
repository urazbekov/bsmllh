...
\nonumber
 \end{align}
Based on this expression the density function $\rho_k(R)$ of the nucleon $k$ has form
\begin{align}
\rho^{(k)}(R)=&\sum_{\gamma} \rho^{(k)}_\gamma(R),
\end{align}
where
\begin{align}
\rho^k_\gamma(R)=\sum_{ij} 
C_iC_j  
\left( y_0^{(k)} \right)^{2l}
R^{2l} 
&\exp \left( -\tfrac{1}{2} {y_0^{(k)}}^2 \beta^{(k)}_{ij} R^2 \right)
...
where
\begin{align}
\rho^{(k)}_\gamma(R)= 4 \pi \exp\left(-\gamma_0 R^2 \right)
\sum_{ij} C_i C_j 
