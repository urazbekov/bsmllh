...
Consider the three body system containing one boson, $k$, and two fermions, $p$ and $q$. 
 A total wave function of this system with total spin $ J $ and spin projection $ M_J $ can be represented as
 \begin{equation}
 \Psi^{JM_J}_{tot}= \phi_{\alpha}\left( {\bf r} \right) 
 \Psi^{JM_J}\left( {\bf x}_k, {\bf y}_k \right).
 \label{totwf}
 \end{equation}
Here, $\phi_{\alpha}\left( {\bf r} \right)$ -- internal wave function of the $\alpha$-particle, $\Psi^{JM_J}\left( {\bf x}_k, {\bf y}_k \right)$ -- wave function of the relative motion depending on the Jacobi coordinates $ {\bf x}_k $ and $ {\bf y}_k $. The vector $ {\bf x}_k $ is a vector of the relative distance between the pair of particles $ pq $ and $ k $, and $ {\bf y}_k $ is the vector of the relative distance between the center of mass of the pair $ pq $ and the particle $ k $. The wave function of relative motion may be expanded into a components as
\begin{equation}
\Psi^{JM_J}\left( {\bf x}_k, {\bf y}_k \right)=\sum_\gamma\Psi^{JM_J}_\gamma\left( {\bf x}_k, {\bf y}_k \right),
\end{equation}
...
\begin{equation}
\Psi^{JM_J}_\gamma\left( {\bf x}_k, {\bf y}_k \right) = \left[
\Phi_L^\gamma \left({\bf x}_k, {\bf y}_k \right) \times
\chi_S(pq)
\right]_{JM_J}
\end{equation}
The spin function of the pair $pq$ is given by
\begin{equation}
\chi_{SM_S}(pq)=\left[ \chi_{\tfrac{1}{2}}(p) \times \chi_{\tfrac{1}{2}}(q)	\right]_{SM_S}.
\end{equation}
The spatial part of the wave function is then expressed as
\begin{align}
\Phi_{LM_L}^\gamma \left({\bf x}_k, {\bf y}_k \right)=&
x^\lambda y^l 
\sum_i C_i 
exp\left( - \alpha_i^{(k)} x^2_k - \beta_i^{(k)} y^2_k \right)~
\times \nonumber \\
& \times \left[ 
Y_\lambda \left(\hat{x}_k \right) \times Y_l \left(\hat{y}_k \right)
\right]_{LM_L}.
\end{align}
Here, the coefficients $ C_{i} $ are the parameters of the wave function expansion and are found as a result of solving the generalized eigenvalue problem,  $ \alpha^{(k)} _ {i} $, $ \beta^{(k)}_{i}$ are the linear parameters of the three-body wave function. 
