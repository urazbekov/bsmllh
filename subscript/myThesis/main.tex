...
The contributions of the transfer of $\alpha$-clusters and nucleons of $^6$He, $^6$Li and $^9$Be are demonstrated. Obtained results based on the proposed theoretical approach reflect the cluster structure of the exploring nuclei.
...
In addition, I would like to address my gratitude to Prof. Vladimir Kukulin (Moscow Sate University), who gave advice on a scientific career, Prof Sayabek Sakhiyev (Abay State University), who gave not only scientific guides but also about life.
...
Last three chapters present the results of calculations. 
...
\begin{equation}
\mathcal{W}  = \sum_{\gamma} \mathcal{W}^{(k)}_{\gamma}.    
\end{equation}
...
\label{rms_r_square}
\end{equation}
where $r=-y_0^{(k)} { y}_k$. This method gives results in easier way, but not goes into the density distribution function of the $\alpha$-clusters.
But, eventually, the rms radii calculated with both Eq. (\ref{rms_general}) and 
(\ref{rms_r_square}) give the same results. More detailed derivation of Eq. (\ref{rms_r_square}) are provided in Ref. \cite{voronchev1982variational}.   
...
This work is dedicated to study the structure of weakly bound nuclei, such \he, \li and \be, and its impact on the direct nuclear reactions. 
 The nuclei are considered as the three body system: \he =$\alpha+2N$, \li$=\alpha+2N$ and \be$=2\alpha+N$. 
 Their wave functions are chosen to be in the form of Gaussian basis. 
 The three body wave function are obtained within the stochastic variational method. 
 Variational calculations underestimate the three body eigen-energies than experimental values.
  Such results point to presence of additional natures. 
  In particular, the $NNN$-forces and other cluster configurations are not implemented explicitly in the current model.
   Taking into account such kind of specificities it may contribute to the properties of the considering nuclear systems.
...
Obtained values on the both charge and matter radii are in good agreement with the available estimations from the well-known sources.
...
By using them we could extract the optical potential parameters, the spectroscopic informations of the three cluster configurations. 
Obtained optical potentials in calculations of elastic scattering and elastic transfer give good agreement of corresponding differential cross sections with experimental data.
...
Analysis based on the CRC calculations show that the major contribution to the elastic transfer cross section is resulted from the di-nucleon transfer channel, whereas the sequential transfer is lower one order of magnitude.
This confirms validity of the three body model, and it is compatible with the conclusions of other authors from \cite{khoa2004di, oganessian1999dynamics}. 
 

The deuteron-induced reactions on a ${}^9$Be target at the collision energies 19.5 and 35 MeV are studied.
...
It is due to the   taking into account all possible couplings including the spin-reorientations. 
The spin reorientation coupling allows, particularly, to improve agreement with data significantly for the \be($d$,$p$)${}^{10}$Be${}_{gs}$ reaction. 
 Such kind of effects occurs when a strong deformed and hight spin valued nucleus, like \be, is involved. 

The theoretical analysis of the nuclear reaction ${}^9$Be($d$, $\alpha$)${}^7$Li$_{gs}$ shows that it goes basically through three reaction mechanisms, which are the transfer mechanism of $n$-$p$,  the simultaneous and sequential transfer of $^5$He.  It has became known that the ${}^5$He heavy cluster is transferred mainly simultaneously at the backward scattering angles, and the contribution of its sequential transfer is an order of magnitude lower.
The $n$-$p$ transfer mechanism turned out to be competitive to the $d$ transfer.
 Based on these observations from studying the reactions $d~+$ $^9$Be nuclear, it can be concluded that the $^9$Be  nucleus has cluster structure.
...
%\include{Appendices/Appendix_RMS}
