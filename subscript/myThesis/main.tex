...
However, there are also stable nuclei with prominent cluster structures. These include such nuclei as \li, \be, $ ^{11}$B and others. Numerous experimental data \cite{canto2006fusion, brown2007, papka2007} allow us to treat such nuclei as multi-cluster systems, including tightly bound $\alpha$-clusters and valence nucleons
...
\nonumber \\
=&~ \mathcal{I}_k \left( \lambda,l,\alpha_{ij}^{(k)},\beta_{ij}^{(k)} \right) ,
\end{align}
where $\mathcal{I}_k$ may be expressed as  
\begin{equation}
\mathcal{I}_k \left( \lambda,l,\alpha_{ij}^{(k)},\beta_{ij}^{(k)} \right)= 2^{1+\lambda+l}\frac{\Gamma \left( \frac{3}{2}+\lambda \right) \Gamma \left( \frac{3}{2}+l \right) }{ \left( \alpha_{ij}^{(k)} \right) ^{\frac{3}{2}+\lambda} \left( \beta_{ij}^{(k)} \right) ^{\frac{3}{2}+l}} .
...
& = \mathcal{I}_q(\lambda, l, n, \alpha, \beta, |\rho|) 
\end{align}
where, $\mathcal{I}_q$ is
\begin{align}
\mathcal{I}_q(\lambda, l, n, \alpha, \beta, |\rho|) =& \sqrt{\frac{\pi}{8}}(2l)!!~ \Gamma(l+n+\tfrac{3}{2})~|\rho|^{n} ~\beta^{-l-n-\tfrac{3}{2}} \times \nonumber \\
...
A_{\gamma\tilde{\gamma}}^{ {\bf Q}_{ij}^{(kq)} }
 A_{\gamma\tilde{\gamma}^{\prime}}^{ {\bf Q}_{ij}^{(kq)} } \mathcal{I}_k \left( \tilde{\lambda}, \tilde{l}, \alpha^{(q)}_{ij}, \left(  \beta^{(q)}_{ij} - \tfrac{\left(\rho^{(q)}_{ij}\right)^2}{\alpha^{(q)}_{ij}} \right) \right) \delta_{\tilde{\gamma}\tilde{\gamma}^{\prime}}.
...
\left( A_{\gamma\tilde{\gamma}}^{ {\bf Q}_{ij}^{(kq)}} \right)^2
 \times\nonumber \\
 & \times
  \mathcal{I} \left( \tilde{\lambda}, \tilde{l}, \alpha^{(q)}_{ij}, \left(  \beta^{(q)}_{ij} - \tfrac{\left(\rho^{(q)}_{ij}\right)^2}{\alpha^{(q)}_{ij}} \right) \right).
\end{align}  
...
\left({\bf Q}_{11} \right)^{\lambda_1} 
\left({\bf Q}_{12} \right)^{\lambda_2} 
\left({\bf Q}_{21} \right)^{l_1} 
\left({\bf Q}_{22} \right)^{l_2} 
\times \nonumber
\\
& \times {E}^{\lambda_1 \lambda_2 \lambda l_1 l_2 l L}_{\tilde{\lambda} \tilde{l} } \mathcal{D}(\lambda,\lambda_1,\lambda_2) \mathcal{D}(l,l_1,l_2)
\times  \nonumber    \\
...
\times  \nonumber    \\
 \times 
 \mathcal{I} & \left( \lambda + l - \tfrac{1}{2} \ell, 
~0, 
