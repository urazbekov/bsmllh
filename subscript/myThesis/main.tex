...
The understanding and naming of modern objects such as molecules and atoms began in ancient antiquity.
 The meaning of the word atom, in Greek $\alpha \tau o \mu \varsigma$, is an indivisible, or uncut particle of matter. 
 Modern science has gone much deeper and determined that the atom is actually a divisible particle. 
...
Experimental techniques have also been successfully developed in the field of the production of secondary beams. 
Secondary beams, being rare nuclei, allow studying their properties and characteristics. 
These obtained data in the course of the study pose a challenge to modern theoretical and experimental physics.

In recent years, the study of light weakly bound nuclei  has not lost interest due to the successful development of experimental facility \cite{ter2004radioactive, yano2007riken, gade2016nscl, hong2014plan, fukuda2013identification}. 
...
The decay of the alpha particle itself as a single system means that it can be found in the uranium nucleus as a separate subsystem.
The first quantitative theory of  decay was developed by the Soviet and American physicist Georgy Gamov (for more information, see \cite{stuewer1986gamow}). In addition to decay by $\alpha$ - particles, there is also cluster decay, where the separated particle can be a nucleus with twelve nucleons -- $ ^ {12} $ C \cite{poenaru2012cluster}.

In light nuclei, the cluster structure can often be clearly manifested in the region of the stability boundary of the nuclear map. Proton-rich nuclei undergo alpha decay. In the case of neutron-rich nuclei, they turn to the isobaric analog state through $\beta$-decay. These series include such nuclei as \he, $^{11}$Li, $^{12}$Be and other exotic nuclei. 
However, there are also stable nuclei with distinctive cluster structures. These include such nuclei as \li, \be, $ ^{11}$B and others.

A visual representation of how these nuclei are arranged in the cluster model is illustrated in Fig \ref{fig:exotic_nuclei}. 
...
Oscillatory functions are written in the form of Slater determinants (DS) with a projection of the total angular momentum fixed for each nucleon.
Calculation of the wave functions of the ground and lowest excited states is reduced to finding the eigenfunctions and eigenvalues of the matrix of the Hamiltonian operator in the basis of the DS. 
Since this matrix is characterized by large dimensions and a high degree of sparseness, the determination of eigenfunctions and eigenvectors is usually carried out by the Lanczos algorithm.
However, the size of the basis grows rapidly with the number of nucleons, and the reliability of the NCSM model calculations in the case of heavy nuclei decreases. At present, the capabilities of modern computational machines make it possible to calculate with sufficient accuracy of nuclei with masses $A \le 16$.
...
The calculations were carried out for positive and negative parities with the AV8 potential.

The microscopic description, that is, taking into account the NN potential, of the two-fragment break-up reaction was presented in the Resonating Group Method (RGM) \cite{tang1978resonating, wheeler1937mathematical}. Generalization of the RGM consists in constructing general, mathematically equivalent methods for the simultaneous calculation of nuclear structure and nuclear reactions. This model allows one to calculate bound states and states of the continuous spectrum in a two-cluster representation with the correct asymptotic behavior of the two-body solution.

In addition to the above theoretical approaches, there is a variational method \cite{kukulin1977stochastic, voronchev1994study, kukulin1986detailed, kukulin1984detailed}. The method uses the Gaussian function as a basis function, which is convenient for deriving analytically matrix elements of the Hamiltonian. The Hamiltonian contains a Pauli projector, which removes forbidden states from the problem of finding the minimum energy. The Schrödinger equation for the three-body system is solved by the method of the Hill-Wheeler intergal equation. The reduced matrix form of the equation is solved by the generalized eigenvalue problem. This method describes well the internal structures of \he, \li and \be. In particular, the static characteristics, geometric structures and component weights are in good agreement with the methods, where they are obtained by real solutions of three-body problems. In this work the wave functions obtained by this method are used.

In addition to studying internal structures, there are several widely used methods for representing quantum scattering states. To describe the state of elastic scattering in the field of a complex potential, one usually uses the Optical Model, first presented in Ref. \cite{feshbach1958optical}. Within the framework of this model, the nucleus is considered as a transparent substance that has the ability to absorb. In practice, this method describes the experimental data well. However, there are several disadvantages. One of them is not a single choice of optical potentials. Another follows from the ambiguity that the model can pick up a potential with  an unrealistic potential depth in the concrete.

To study the inelastic channels, transfer reactions and their use to describe experimental data the Distorted Wave Born Approximation (DWBA) method has been successfully applied \cite{satchler1960gamma}. The distorted wave method is a good tool for describing nuclear reactions with products registered in the ground state. For transfer channels with excited states, calculations using the DWBA method in many cases give disagreement with the experimental data. In this case, the disagreement is explained by the fact that the wave function of the bound state for the output channel corresponds to a discrete eigenvalue, while the state of this excited nucleus in the experimental data has a resonance with a certain width in the order of several $MeV$. An adequate theoretical analysis of such problems requires other approaches that require taking into account the couplings between channels.

There is an overlap of transferred particles or moments between the reaction channels. Therefore, methods such as Coupled Channels (CC), Couples Reaction Channels (CRC), Continuum Discretized Coupled Channels (CDCC) are used \cite{satchler1983}. The CC method is usually applied to inelastic scatterings, while the CRC is for transfer reactions.
In accordance with the name of the CDCC, the essence of the method consists in discretizing the continuum and in coupling them with the reaction channels. As in the DWBA method, CDCC requires data which take into account the internal structure of colliding nuclei. There is another method developed on the basis of the CDCC method, this is XCDCC \cite{de2014continuum}. The addition to the name “X” means taking into account the excited state of the target nucleus. Recently, a continuum discretization method was proposed based on imaginary correlated gaussian functions \cite{sailaubek2018charged}. This approach successfully describes the displacement phases of elastic scattering of charged particles and reproduces the real Coulomb functions well.
...
\begin{figure}
\centering
\includegraphics[scale=1]{li_elastic}
\includegraphics[scale=1]{li_elastic_transfer}\\
$~~~~~~a)~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~b)$
\decoRule
