...
\chapter{Introduction to the nuclear reactions and cluster phenomenon} % Main chapter title
...
\chapter{Theoretical models describing the nuclear reactions} % Main chapter title
...
\begin{align}
V^V\left( r \right) = & N_0 V^{DF} \left( r \right) \nonumber \\ 
...
\end{align}
where $r_{aA}=\vert {\bf r} + {\bf r}_A - {\bf r}_a \vert$ (see Fig.~\ref{fig:df_scheme}). The $N_0$ normalization parameter is usually fitted in accordance with the dynamics of nuclear reaction of elastic scattering.
...
The DF potential may be calculated using the effective M3Y-Paris nucleon-nucleon potential and the nuclear-matter-densities of projectile and target nuclei. 
For the particles $t$, $^3$He, $\alpha$, which have simple structure, the density distribution of nuclear matter can be represented as parametrized Gaussian function as follow
 \begin{equation}
 \rho \left( r \right) = N {\mathrm{exp}}\left( - a r^2 \right)
 \end{equation}
 where the parameters $a$ and $A$ are defined in condition to reproduce the rms matter radii
 \begin{equation}
 a = \frac{3}{2\langle r_{m}^{2} \rangle}, 
 N = A  \left( \frac{a}{\pi} \right)^{3/2}
 \end{equation}
...
\begin{table*}[tp]
\footnotesize
\caption{\label{dbe_SA} Spectroscopic amplitudes used in CRC calculations for the Composite = Core + Nucleon system. The one-nucleon spectroscopic amplitudes have been calculated by means of the $ANTOINE$ code \cite{antoine}. The alpha spectroscopic amplitudes were taken from  \cite{volya, volya2017}. }
...
\caption{\label{dbe_fig11} Contributions of the different mechanisms to the cross section of the ${}^9$Be($d,\alpha$)${}^7$Li$_{g.s.}$ reaction. See Fig.~\ref{fig9} for explanation of the curve notations.}
\end{figure}	
